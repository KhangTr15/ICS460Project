...
//https://github.com/tomersimis/Go-Back-N/blob/master/Receiver.java
	...
	public void startUDPServer(){
		...
		try {
			...
			while(!shutdowned){
				...
				if(pIn!=null && pIn.getSeqno()>STARTING_SEQ_NUM && pIn.getSeqno()!= END_OF_FLAG){
				...
				System.out.println("111111HJEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE "+pIn.getSeqno()+ " Does The Seq Num match the expected Seq Num? "+(pIn.getSeqno()==expectedSeqNum)+ " Expected Seq Num"+ expectedSeqNum);
				System.out.println("Something Khang added" + received.peek());
				
				
				if(pIn!= null && pIn.getSeqno()==expectedSeqNum){
					System.out.println("22222HJEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE "+pIn.getSeqno()+ " Does The Seq Num match the expected Seq Num? "+(pIn.getSeqno()==expectedSeqNum)+ " Expected Seq Num"+ expectedSeqNum);
					received.add(pIn);
					out(Calendar.getInstance().getTime()+"\n"+"Server: I got RIGHT Data Packet\n"+pIn);
					System.out.println("333332HJEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE "+pIn.getSeqno()+ " Does The Seq Num match the expected Seq Num? "+(pIn.getSeqno()==expectedSeqNum)+ " Expected Seq Num"+ expectedSeqNum);
					
					try {
						if(writer!=null && pIn!=null){
							System.out.println("Succesfully wrote to Writer "+pIn.getSeqno()+ " Does The Seq Num match the expected Seq Num? "+(pIn.getSeqno()==expectedSeqNum)+ " Expected Seq Num"+ expectedSeqNum);
					...
					try {
						System.out.println("55555555HJEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE "+pIn.getSeqno()+ " Does The Seq Num match the expected Seq Num? "+(pIn.getSeqno()==expectedSeqNum)+ " Expected Seq Num"+ expectedSeqNum);
					...
					System.out.println("Managed to Re-send packet "+pIn.getSeqno()+ " Does The Seq Num match the expected Seq Num? "+(pIn.getSeqno()==expectedSeqNum)+ " Expected Seq Num"+ expectedSeqNum);
			...
			if(pIn!=null)
				System.out.println("77777777777777777777777777772HJEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE"+pIn.getSeqno()+ " Does The Seq Num match the expected Seq Num? "+(pIn.getSeqno()==expectedSeqNum)+ " Expected Seq Num"+ expectedSeqNum);
	...
	private void respond(DatagramSocket socket, DatagramPacket packet, Packet askPacket) throws IOException {
		...
		out(Calendar.getInstance().getTime()+"\n+Server: Sending Ack "
				+askPacket.getAckno()+" ("+outByteArr.length+")bytes to send>>>>\n"+askPacket);
	...
	private boolean checkChekSum(Packet packet) {
		return packet.getCksum() == calcCheckSum(packet);
	}
	private short calcCheckSum(Packet packet) {
